...
char** _str_split(char* str, const char delim, int *n) {
    ...
    // delim是分隔符
    // count用来统计有多少个操作符
    // last_comma 用来统计最后一个分隔符
    last_comma = str; // 我加的一行，用来Debug
    while (*tmp) {
        if (delim == *tmp) {
            count++;
            last_comma = tmp;
    ...
    // printf("count = %ld\n", count);
    count += last_comma < (str + strlen(str) - 1); // 看看last_comma是否超过这条指令的最末端
    *n = count;
    ...
    if (result) {
        ...
        // printf("idx=%ld    count=%ld\n", idx, count);
        assert(idx == count); // TODO: 问题所在
        *(result + idx) = 0;
    }
    return result;
}
